<?php
	...
	$sql2 = "SELECT * FROM user_acc,reported_intern WHERE reported_intern.report_count = '1' AND reported_intern.user_acc_id = user_acc.user_acc_id AND  reported_intern.team_name ='$team_name'";
	...
	if ($result2->num_rows > 0) {
	    ...
	    while($row = $result2->fetch_array()) {
            ...
            if($count >= 4 && $count < 6 )
            {
                $listitem["Status"] = "<label style='color:black' class='badge badge-warning'>ACCOUNT WARNING</label>";
            }else if($count >= 6)
            {
               $listitem["Status"] = "<label class='badge badge-danger'>ACCOUNT BANNED</label>"; 
            }
            else{
                $listitem["Status"] = "<label class='badge badge-info'>ACCOUNT REPORTED</label>";
            }
