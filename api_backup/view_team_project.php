<?php
    ...
    $sql = "SELECT * FROM team, user_acc, team_project where team.leader_id=user_acc.user_acc_id AND team_project.team_name=team.team_name AND team_project.user_acc_id=user_acc.user_acc_id AND team_project.user_acc_id='".$user_acc_id."'";
      ...
      while($row = $result->fetch_array()) {
        ...
        if($row["co_leader_id"]==0)
        {
            ...
            $result1 = $conn->query($sql1);
                ...
                $view_team_project["Team Leader"] = $row["firstname"]." ".$row["middle_name"]." ".$row["lastname"];
                $view_team_project["Co Leader"] = $row1["firstname"]." ".$row1["middle_name"]." ".$row1["lastname"];
                $view_team_project["Task Name"] = $row["task_name"];
                $view_team_project["File Format"] = $row["file_formats"];
                $view_team_project["Date Assigned"] = $date_assigned1;
                $view_team_project["Date Submitted"] = $date_submitted1;
